...
master2=192.168.10.131
master3=192.168.10.132
worker1=192.168.10.133
worker2=192.168.10.134
worker3=192.168.10.135
...
# Array of all nodes
allNodes=($master1 $master2 $master3 $worker1 $worker2 $worker3)

# Array of all minus master
allnomaster1=($master2 $master3 $worker1 $worker2 $worker3)

#Loadbalancer IP range
lbrange=192.168.10.52-192.168.10.72

# SSH certificate name variable
# This SSH certificate is already installed on all nodes via TF deployment on PM
certName=id_ed25519_vscode
...
# Step 1: Bootstrap First k3s Master Node
mkdir ~/.kube
k3sup install \
  --ip $master1 \
  --user $user \
